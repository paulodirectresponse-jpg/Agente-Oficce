...
export function ConfiguracoesView({providers}:{providers:UniversalProvider[]}){
  ...
  const updateRef=(key:'principal'|'fast'|'deep',provider_id:string)=>{
    ...
    setOrchestrator({...orchestrator,[key]:provider_id?{provider_id,model_id:model?.model_id??null}:key==='principal'?{provider_id:null,model_id:null}:null});
  };

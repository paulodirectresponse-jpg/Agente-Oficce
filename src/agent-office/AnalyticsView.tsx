...
import type { MonitoramentoRange, MonitoramentoSnapshot, Project } from './types.js';
...
export function MonitoramentoView(){
  const [data,setData]=useState<MonitoramentoSnapshot|null>(null);
  const [projects,setProjects]=useState<Project[]>([]);
  const [range,setRange]=useState<MonitoramentoRange>('7d');
  const [projectId,setProjectId]=useState('');
  const [workerFilter,setMembroFilter]=useState('');
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const [snapshot,list]=await Promise.all([api.getMonitoramentoV3(filters),api.listProjects()]);
  ...
  return <div className="analytics-page">
    <header className="analytics-header">
      <div><span className="office-kicker">Inteligência operacional</span><h1>Monitoramento</h1><p>Métricas observáveis do Agent Office, sem scores artificiais e sem dupla contagem de Team Execuções.</p></div>
      <div className="analytics-filters">
        <select value={range} onChange={e=>setRange(e.target.value as MonitoramentoRange)}>
        ...
        <select value={workerFilter} onChange={e=>setMembroFilter(e.target.value)}>
        ...
        <select value={providerId} onChange={e=>{setProviderId(e.target.value);setModelId('')}}>
          <option value="">Todos os IAs</option>{data?.providers.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
        </select>
    ...
    {!data&&loading&&<div className="empty-state">Carregando Monitoramento…</div>}
    ...
    {data&&<>
      ...
      {tab==='overview'&&<div className="analytics-stack">
        <div className="analytics-kpis">
          <div><span>Execuções</span><strong>{fmt(data.overview.execuções)}</strong><small>{data.overview.completed_execuções} concluídos · {data.overview.failed_execuções} falharam</small></div>
          <div><span>Taxa de sucesso</span><strong>{pct(data.overview.success_rate)}</strong><small>somente Execuções terminais</small></div>
          ...
          <div><span>Falhas operacionais</span><strong>{data.overview.operational_failures}</strong><small>{data.overview.cancelled_execuções} Execuções cancelados</small></div>
        </div>
        <section className="analytics-card">
          ...
          <div className="analytics-chart">
            {data.timeseries.map(point=><div className="analytics-bar-col" key={point.bucket} title={`${point.bucket} · ${point.total_tokens} tokens · ${point.execuções} execuções`}>
              ...
              <strong>{point.execuções}</strong><span>{range==='24h'?point.bucket.slice(11,16):point.bucket.slice(5)}</span>
            </div>)}
      ...
      {tab==='workers'&&<div className="analytics-stack"><section className="analytics-card">
        <div className="analytics-card-head"><div><span className="office-kicker">Equipe</span><h2>Equipe e Subagents</h2></div><small>qualidade e operação permanecem separadas</small></div>
        <div className="analytics-table-wrap"><table className="data-table analytics-table"><thead><tr><th>Membro</th><th>Tipo</th><th>Execuções</th><th>Sucesso</th><th>Primeira tentativa</th><th>Retrabalho</th><th>Tokens</th><th>Custo</th><th>Latência</th></tr></thead><tbody>
          {data.workers.map(w=><tr key={w.worker_kind+':'+w.id}><td><strong>{w.name}</strong><small>{w.role||'—'}</small></td><td><span className={'analytics-kind '+w.worker_kind}>{w.worker_kind}</span></td><td>{w.execuções}</td><td>{pct(w.success_rate)}</td><td>{pct(w.first_pass_rate)}</td><td>{pct(w.rework_rate)}</td><td>{fmt(w.total_tokens)}</td><td>{money(w.cost_usd)}<small>{w.cost_coverage_pct.toFixed(0)}% cobertura</small></td><td>{ms(w.average_run_duration_ms)}</td></tr>)}
        </tbody></table></div>
      </section></div>}
      ...
      {tab==='execution'&&<div className="analytics-stack">
        <div className="analytics-kpis">
          ...
          <div><span>Taxa de repetição</span><strong>{pct(data.execution.retry_rate)}</strong><small>{data.execution.retry_tentativas} tentativas extras</small></div>
          ...
          <div><span>Duração média</span><strong>{ms(data.execution.average_attempt_duration_ms)}</strong><small>{data.execution.tentativas} tentativas</small></div>
        </div>
        <section className="analytics-quality">
      ...
      {tab==='providers'&&<div className="analytics-provider-grid">
        {data.providers.map(p=><section className="analytics-card analytics-provider-card" key={p.id}>
          <div className="analytics-card-head"><div><span className="office-kicker">{p.operational_status}</span><h2>{p.name}</h2></div><span className={'mini-status '+(p.health_status==='healthy'?'online':p.enabled?'desconhecido':'offline')}/></div>
      ...
      {tab==='orchestrator'&&<div className="analytics-stack">
        <div className="analytics-kpis">
          ...
          <div><span>Sucesso</span><strong>{pct(data.orchestrator.success_rate)}</strong><small>resultado do roteamento</small></div>
          ...
          <div><span>Tokens</span><strong>{fmt(data.orchestrator.input_tokens+data.orchestrator.output_tokens)}</strong><small>somente Orquestração</small></div>
        ...
        <section className="analytics-quality">
          ...
          <div><strong>Sucesso pós-roteamento</strong><span>{pct(data.orchestrator.execution_outcome.success_rate)}</span></div>
        </section>
      ...
      {tab==='usage'&&<div className="analytics-stack">
        <div className="analytics-kpis">
          ...
          <div><span>Custo acumulado conhecido</span><strong>{money(data.overview.cost_usd)}</strong><small>nunca substitui desconhecido por zero</small></div>
          <div><span>Cobertura</span><strong>{data.overview.cost_coverage_pct.toFixed(1)}%</strong><small>{data.overview.cost_known_events} conhecidos · {data.overview.cost_desconhecido_events} desconhecido</small></div>
          <div><span>Duração média worker</span><strong>{ms(data.overview.average_duration_ms)}</strong><small>{data.overview.usage_events} eventos de uso</small></div>
        </div>
        <section className="analytics-card"><div className="analytics-card-head"><div><span className="office-kicker">Qualidade dos dados</span><h2>Como estes números são calculados</h2></div></div>
          ...
          <div className="analytics-coverage"><span>Custo informado: <strong>{data.overview.cost_reported_events}</strong></span><span>Custo estimado: <strong>{data.overview.cost_estimated_events}</strong></span><span>Desconhecido: <strong>{data.overview.cost_desconhecido_events}</strong></span></div>
        </section>
      </div>}
    </>}
  </div>

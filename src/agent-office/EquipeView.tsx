        ...
        {(selected||creating)&&<>
          ...
          <div className="team-v2-panel">
            {tab==='geral'&&<>
              ...
              {!creating&&overview&&<div className="team-v2-summary"><div><span>Disponibilidade</span><strong>{readinessLabel(overview.readiness)}</strong></div><div><span>Sucesso operacional</span><strong>{overview.performance.operational_success_rate==null?'—':overview.performance.operational_success_rate.toFixed(1)+'%'}</strong></div><div><span>Retrabalho</span><strong>{overview.performance.rework_rate==null?'—':overview.performance.rework_rate.toFixed(1)+'%'}</strong></div></div>}
            </>}

...
const STATE_LABELS:Record<RoomVisualState,string>={offline:'Offline',idle:'Disponível',resting:'Em espera',thinking:'Pensando',planning:'Planejando',responding:'Respondendo',coding:'Programando',testing:'Testando',reviewing:'Revisando',waiting:'Aguardando',blocked:'Bloqueado',error:'Erro'};
function normalized(value:string):RoomVisualState{if(['offline','idle','resting','thinking','planning','responding','coding','testing','reviewing','waiting','blocked','error'].includes(value))return value as RoomVisualState;if(value==='running'||value==='working')return'responding';return'idle'}
...
export function OfficeMap({agents,providers,states,liveStates,target,onTarget,lastHandoff}:{agents:AgentProfile[];providers:UniversalProvider[];states:AgentState[];liveStates:Record<string,{state:string;activity:string;updated_at:string}>;target:string;onTarget:(value:string)=>void;lastHandoff:{from:string;to:string}|null}){
  ...
  return <div className="room-map" aria-label="Escritório operacional dos Agents">
    <div className="room-wall"><div className="room-window"><span/><span/><span/></div><div className="room-brand"><strong>AGENT OFFICE</strong><span>operational workspace</span></div><div className="room-clock">AO</div></div>
    <div className="room-floor">
      <div className="room-zone zone-project"><span className="room-zone-label">Projeto</span><div className="project-board"><i/><i/><i/></div><div className="project-table"><span/><span/><span/></div></div>
      <div className="room-zone zone-meeting"><span className="room-zone-label">Reunião</span><div className="meeting-table"><i/><i/><i/><i/></div></div>
      <div className="room-zone zone-lounge"><span className="room-zone-label">Lounge</span><div className="lounge-sofa"><i/><i/><i/></div><div className="lounge-rug"/><div className="coffee-table"/></div>
      <div className="room-ops-board"><strong>Operações</strong><span/><span/><span/></div><div className="room-plant plant-left"/><div className="room-plant plant-right"/>
      <div className={'room-stations count-'+Math.min(views.length,12)}>{views.map(agent=><AgentStation key={agent.id} agent={agent} onSelect={()=>onTarget(agent.selected?'auto':agent.id)}/>)}{!views.length&&<div className="room-empty"><strong>Nenhum Agent disponível</strong><span>Crie ou habilite um Agent para ocupar o escritório.</span></div>}</div>

...
describe('Agent Office local database', () => {
  ...
  it('upgrades a migration-12 database through migration 23 without losing existing rows', () => {
    ...
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='agent_performance_events'").get()).toEqual({ name: 'agent_performance_events' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='team_rooms'").get()).toEqual({ name: 'team_rooms' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='team_room_entries'").get()).toEqual({ name: 'team_room_entries' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='subagents'").get()).toEqual({ name: 'subagents' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='subagent_capabilities'").get()).toEqual({ name: 'subagent_capabilities' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='subagent_usage_snapshots'").get()).toEqual({ name: 'subagent_usage_snapshots' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='workforce_team_members'").get()).toEqual({ name: 'workforce_team_members' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='workforce_resource_metadata'").get()).toEqual({ name: 'workforce_resource_metadata' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='runtime_worker_delegations'").get()).toEqual({ name: 'runtime_worker_delegations' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='workspace_run_commands'").get()).toEqual({ name: 'workspace_run_commands' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='workspace_run_baselines'").get()).toEqual({ name: 'workspace_run_baselines' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='preview_sessions'").get()).toEqual({ name: 'preview_sessions' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='project_decisions'").get()).toEqual({ name: 'project_decisions' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='project_blockers'").get()).toEqual({ name: 'project_blockers' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='project_results'").get()).toEqual({ name: 'project_results' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='integration_connections'").get()).toEqual({ name: 'integration_connections' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='integration_capabilities'").get()).toEqual({ name: 'integration_capabilities' });
    expect(upgraded.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='project_integration_bindings'").get()).toEqual({ name: 'project_integration_bindings' });

...
describe('Agent Office local database', () => {
  ...
  it('upgrades a migration-12 database through migration 21 without losing existing rows', () => {
    ...
    expect(upgraded.connection.prepare('SELECT MAX(version) version FROM schema_migrations').get()).toEqual({ version: 21 });

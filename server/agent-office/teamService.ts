...
export interface TeamInput {
  name:string; slug:string; purpose?:string; type?:TeamType; lead_agent_id?:string|null;
...
export class TeamService {
 ...
 createPermanent(input:TeamInput,actor='user:manual'){
  ...
  this.validateLead(input.lead_agent_id??null);
  const teamId=id(),t=now(),policy=normalizePolicy(input.policy);
  const tx=this.db.transaction(()=>{
   this.db.prepare(`INSERT INTO teams(id,name,slug,purpose,type,lead_agent_id,enabled,max_parallelism,max_delegation_depth,allow_external_borrowing,proposal_policy,metadata_json,current_version,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,1,?,?)`).run(teamId,name,slug,input.purpose??'',type,input.lead_agent_id??null,input.enabled===false?0:1,Math.max(1,Math.floor(input.max_parallelism??3)),Math.max(0,Math.floor(input.max_delegation_depth??2)),input.allow_external_borrowing?1:0,input.proposal_policy??'manual',JSON.stringify(input.metadata??{}),t,t);
   this.writePolicy(teamId,policy,t);this.snapshot(teamId);
 ...
 listAgentTeams(agentId:string){return (this.db.prepare(`SELECT t.* FROM teams t JOIN team_members tm ON tm.team_id=t.id WHERE tm.agent_id=? ORDER BY t.name`).all(agentId) as any[]).map(r=>this.hydrate(r))}
 ...
 createDynamic(input:DynamicTeamInput){
  ...
  const members=[...new Set(input.member_ids)];for(const a of members){const row=enabledAgent(this.db,a);if(!row||!row.enabled||!row.provider_enabled||!row.model_enabled)throw new Error('DYNAMIC_TEAM_MEMBER_UNAVAILABLE')}

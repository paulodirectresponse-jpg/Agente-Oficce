...
import { SubagentService, type SubagentInput } from './subagentService.js';

export type TeamType='permanent'|'system';
export type TeamKind='permanent'|'dynamic';
export interface TeamPolicyInput {
  allowed_tools?:string[]; permissions?:string[]; delegation_permissions?:string[];
  approval_mode?:'safe'|'manual'|'auto'; budget_defaults?:Record<string,number>; metadata?:Record<string,unknown>;
}
export interface TeamInput {
  name:string; slug:string; purpose?:string; type?:TeamType; owner_agent_id?:string|null; lead_agent_id?:string|null;
  enabled?:boolean; max_parallelism?:number; max_delegation_depth?:number; allow_external_borrowing?:boolean;
...
function normalizePolicy(input:TeamPolicyInput={}){
  return{
    allowed_tools:uniq(input.allowed_tools),permissions:uniq(input.permissions),delegation_permissions:uniq(input.delegation_permissions),
...
export class TeamService {
  ...
  createSubagent(teamId:string,input:SubagentInput){
    const team=this.get(teamId);if(!team)throw new Error('TEAM_NOT_FOUND');if(!team.owner_agent_id)throw new Error('SUBAGENT_REQUIRES_OWNED_TEAM');
    const created=new SubagentService(this.db).create(teamId,input);this.bumpVersion(teamId);
    this.appendRoomEntry(teamId,{agent_id:team.owner_agent_id,subagent_id:created.id,entry_type:'activity',content:'Subagent criado.',payload:{subagent_id:created.id,name:created.name}});
    return created;
  }
  updateSubagent(teamId:string,subagentId:string,input:Partial<SubagentInput>){
    ...
    const updated=svc.update(subagentId,input);this.bumpVersion(teamId);
    this.appendRoomEntry(teamId,{subagent_id:subagentId,entry_type:'activity',content:'Subagent atualizado.',payload:{subagent_id:subagentId}});
    return updated;
  }
  removeSubagent(teamId:string,subagentId:string){
    const svc=new SubagentService(this.db),current=svc.get(subagentId);if(!current||current.team_id!==teamId)throw new Error('SUBAGENT_NOT_FOUND');
  ...
  updateRoom(teamId:string,input:{instructions?:string;shared_context?:Record<string,unknown>;memory?:Record<string,unknown>}){
    ...
    this.db.prepare('UPDATE team_rooms SET instructions=?,shared_context_json=?,memory_json=?,updated_at=? WHERE team_id=?').run(
      input.instructions===undefined?cur.instructions:String(input.instructions),JSON.stringify(input.shared_context??cur.shared_context),JSON.stringify(input.memory??cur.memory),now(),teamId
    );this.appendRoomEntry(teamId,{entry_type:'memory',content:'Contexto do Team Room atualizado.'});return this.getRoom(teamId);
  }
  appendRoomEntry(teamId:string,input:{agent_id?:string|null;subagent_id?:string|null;entry_type?:string;content?:string;payload?:Record<string,unknown>}){
    if(!this.get(teamId))throw new Error('TEAM_NOT_FOUND');this.ensureRoom(teamId);
    ...
    const entry={id:id(),team_id:teamId,agent_id:input.agent_id??null,subagent_id:input.subagent_id??null,entry_type:type,content:String(input.content??''),payload:input.payload??{},created_at:now()};
  ...
  createPermanent(input:TeamInput,actor='user:manual'){
    ...
    if(input.owner_agent_id)this.validateOwner(input.owner_agent_id);else this.validateLead(input.lead_agent_id??null);
  ...
  update(teamId:string,input:Partial<TeamInput>){
    ...
    if(input.lead_agent_id!==undefined&&!current.owner_agent_id)this.validateLead(input.lead_agent_id);
    ...
    const tx=this.db.transaction(()=>{
      ...
      if(input.policy)this.writePolicy(teamId,normalizePolicy({...current.policy,...input.policy}),now());this.bumpVersion(teamId);
    });tx.immediate();return this.get(teamId)!;
  }
  disable(teamId:string){return this.update(teamId,{enabled:false})}

  // Legacy V3.6 generic teams only. Owned Agent Teams use true Subagents.
  replaceMembers(teamId:string,members:TeamMemberInput[]){
    const team=this.get(teamId);if(!team)throw new Error('TEAM_NOT_FOUND');if(team.owner_agent_id)throw new Error('OWNED_TEAM_REQUIRES_SUBAGENTS');
    const seen=new Set<string>();for(const m of members){if(seen.has(m.agent_id))throw new Error('TEAM_MEMBER_DUPLICATE');seen.add(m.agent_id);if(!this.db.prepare('SELECT 1 FROM agents WHERE id=?').get(m.agent_id))throw new Error('AGENT_NOT_FOUND')}
    if(team.lead_agent_id&&!seen.has(team.lead_agent_id))throw new Error('TEAM_LEAD_MUST_BE_MEMBER');
    const tx=this.db.transaction(()=>{this.db.prepare('DELETE FROM team_members WHERE team_id=?').run(teamId);const ins=this.db.prepare(`INSERT INTO team_members(team_id,agent_id,role_name,priority,enabled,metadata_json,created_at,updated_at)VALUES(?,?,?,?,?,?,?,?)`);const t=now();for(const m of members)ins.run(teamId,m.agent_id,m.role_name??'',Math.floor(m.priority??0),m.enabled===false?0:1,JSON.stringify(m.metadata??{}),t,t);this.bumpVersion(teamId)});tx.immediate();return this.get(teamId)!;
  }
  ...
  listAgentTeams(agentId:string){return (this.db.prepare(`SELECT DISTINCT t.* FROM teams t LEFT JOIN team_members tm ON tm.team_id=t.id WHERE t.owner_agent_id=? OR (t.owner_agent_id IS NULL AND tm.agent_id=?) ORDER BY t.name`).all(agentId,agentId) as any[]).map(r=>this.hydrate(r))}
  ...
  capabilities(teamId:string,requirements:CapabilityRequirement[]=[]){
    ...
    const specialists:Array<{id:string;kind:'agent'|'subagent';caps:any[]}>=[],ops=new AgentOperationsService(this.db),subs=new SubagentService(this.db);
  ...
  private snapshot(teamId:string){
    const team=this.db.prepare('SELECT * FROM teams WHERE id=?').get(teamId) as any;if(!team)throw new Error('TEAM_NOT_FOUND');const policy=this.db.prepare('SELECT * FROM team_policies WHERE team_id=?').get(teamId) as any;
    const snap={team:{id:team.id,name:team.name,slug:team.slug,purpose:team.purpose,type:team.type,owner_agent_id:team.owner_agent_id??null,lead_agent_id:team.lead_agent_id,enabled:Boolean(team.enabled),max_parallelism:team.max_parallelism,max_delegation_depth:team.max_delegation_depth,allow_external_borrowing:Boolean(team.allow_external_borrowing),proposal_policy:team.proposal_policy,metadata:parse(team.metadata_json,{})},policy:policy?{allowed_tools:parse(policy.allowed_tools_json,[]),permissions:parse(policy.permissions_json,[]),delegation_permissions:parse(policy.delegation_permissions_json,[]),approval_mode:policy.approval_mode,budget_defaults:parse(policy.budget_defaults_json,{}),metadata:parse(policy.metadata_json,{})}:normalizePolicy(),members:team.owner_agent_id?[]:this.listMembers(teamId),subagents:team.owner_agent_id?this.listSubagents(teamId):[],legacy_members:team.owner_agent_id?this.listMembers(teamId):[]};
    this.db.prepare('INSERT INTO team_versions(id,team_id,version,snapshot_json,created_at)VALUES(?,?,?,?,?)').run(id(),teamId,team.current_version,JSON.stringify(snap),now());
  }
  private capCovers(c:any,r:CapabilityRequirement,defs:Map<string,any>){const min=r.minimum??0;const score=(c.verified_score??c.declared_score)*Math.max(.25,c.confidence);if(score<min)return false;if(c.capability_key===r.key)return true;if(r.allow_hierarchy!==false){let d=defs.get(c.capability_key),seen=new Set<string>();while(d?.parent_key&&!seen.has(d.parent_key)){if(d.parent_key===r.key)return true;seen.add(d.parent_key);d=defs.get(d.parent_key)}}return false}
}

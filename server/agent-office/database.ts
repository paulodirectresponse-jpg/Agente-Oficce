...
export const agentOfficeMigrations: Array<{ version: number; sql: string }> = [
  ...
  },
  {
    ...
    sql: `
      ...
      CREATE INDEX IF NOT EXISTS idx_execution_plans_project_status ON execution_plans(project_id,status);
      CREATE TABLE IF NOT EXISTS execution_steps (
        id TEXT PRIMARY KEY,
        plan_id TEXT NOT NULL REFERENCES execution_plans(id) ON DELETE CASCADE,
        ...
        assigned_agent_id TEXT REFERENCES agents(id) ON DELETE SET NULL,
        assigned_team_id TEXT,
        status TEXT NOT NULL CHECK(status IN ('queued','ready','running','blocked','completed','failed','cancelled')),
      ...
      CREATE INDEX IF NOT EXISTS idx_execution_steps_plan_status ON execution_steps(plan_id,status,priority);
      CREATE TABLE IF NOT EXISTS step_dependencies (
        step_id TEXT NOT NULL REFERENCES execution_steps(id) ON DELETE CASCADE,
        depends_on_step_id TEXT NOT NULL REFERENCES execution_steps(id) ON DELETE CASCADE,
        dependency_type TEXT NOT NULL CHECK(dependency_type IN ('hard','artifact','approval')),
        PRIMARY KEY(step_id,depends_on_step_id)
      );
      CREATE INDEX IF NOT EXISTS idx_step_dependencies_reverse ON step_dependencies(depends_on_step_id,step_id);
      CREATE TABLE IF NOT EXISTS step_attempts (
        id TEXT PRIMARY KEY,
        step_id TEXT NOT NULL REFERENCES execution_steps(id) ON DELETE CASCADE,
        ...
        run_id TEXT,
        child_run_id TEXT,
        status TEXT NOT NULL CHECK(status IN ('started','completed','failed','cancelled','timed_out')),
        started_at TEXT NOT NULL,
        ended_at TEXT,
        error_json TEXT,
        result_summary TEXT,
      ...
      CREATE INDEX IF NOT EXISTS idx_step_attempts_step ON step_attempts(step_id,attempt_number);
      CREATE TABLE IF NOT EXISTS execution_artifacts (
        id TEXT PRIMARY KEY,
        plan_id TEXT NOT NULL REFERENCES execution_plans(id) ON DELETE CASCADE,
      ...
      CREATE TABLE IF NOT EXISTS work_packets (
        id TEXT PRIMARY KEY,
        plan_id TEXT NOT NULL REFERENCES execution_plans(id) ON DELETE CASCADE,
        from_step_id TEXT REFERENCES execution_steps(id) ON DELETE SET NULL,
        to_step_id TEXT REFERENCES execution_steps(id) ON DELETE SET NULL,
      ...
      CREATE TABLE IF NOT EXISTS resource_locks (
        ...
        resource_key TEXT NOT NULL,
        mode TEXT NOT NULL CHECK(mode IN ('read','write','exclusive')),
        owner_step_id TEXT REFERENCES execution_steps(id) ON DELETE CASCADE,
        owner_attempt_id TEXT REFERENCES step_attempts(id) ON DELETE CASCADE,
      ...
      CREATE INDEX IF NOT EXISTS idx_resource_locks_resource_status ON resource_locks(project_id,resource_key,status);
    `,
  }
];

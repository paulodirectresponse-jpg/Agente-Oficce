...
export function createExecutionCheckpoint(db:Database,planId:string,reason:string){
 const plan=db.prepare('SELECT id,project_id,version,status,replan_count,budget_json FROM execution_plans WHERE id=?').get(planId) as any;
 ...
 const tx=db.transaction(()=>{const row=db.prepare('SELECT COALESCE(MAX(sequence),0)+1 next FROM execution_checkpoints WHERE plan_id=?').get(planId) as any;const checkpointId=id();db.prepare('INSERT INTO execution_checkpoints(id,plan_id,sequence,reason,snapshot_json,created_at)VALUES(?,?,?,?,?,?)').run(checkpointId,planId,row.next,reason,json(snapshot),now());return{checkpoint_id:checkpointId,sequence:Number(row.next),snapshot}});
 return tx.immediate();
}

...
export function getOrchestratorStatus(db:Database){
  ...
  const stats=db.prepare("SELECT COUNT(*) total,SUM(level_used='deterministic') deterministic,SUM(level_used='fast') fast,SUM(level_used='deep') deep,SUM(level_used='fallback') fallback,AVG(duration_ms) avg_duration_ms,SUM(COALESCE(input_tokens,0)) input_tokens,SUM(COALESCE(output_tokens,0)) output_tokens FROM orchestration_runs WHERE created_at>=datetime('now','-1 day')").get() as any;
  const latest=db.prepare('SELECT created_at FROM orchestration_runs ORDER BY created_at DESC LIMIT 1').get() as any;
  return{settings:s,principal:describe(s.principal),fast:describe(s.fast??s.principal),deep:describe(s.deep??s.principal),stats_24h:{total:Number(stats?.total||0),deterministic:Number(stats?.deterministic||0),fast:Number(stats?.fast||0),deep:Number(stats?.deep||0),fallback:Number(stats?.fallback||0),avg_duration_ms:Math.round(Number(stats?.avg_duration_ms||0)),input_tokens:Number(stats?.input_tokens||0),output_tokens:Number(stats?.output_tokens||0)},latest_run_at:latest?.created_at??null,healthy:s.enabled&&Boolean(s.principal.provider_id&&s.principal.model_id)}
}
